...
\]
as the set of processors that contain $x_i$ in one of their triangles. Each vertex also has an \emph{owner}$,  \Pi_{owner}(i)$, which is just some processor in ons processor set. We chose $\Pi_{owner}(i) := \min \Pi(i)$. Remember that every vertex was identified with a degree of freedom. As vertices might be shared with different processors, this induces a different kind of vector distrbution than the one described in \cite[Ch.~4]{biss04}: all the processors in a set must have a copy of the corresponding vector value.
