...
\begin{figure}
  ...
  \caption{Percentage of total time spent doing local computations (left) and finding the global solution from the local components (right).}
  \label{fig:nonsig}
\end{figure}
...
Taking $n \in \{2048, 4096\}$ and $\delta \in \{0.1, 0.2\}$, we get a few sample matrices. Looking at $t_{mv}$ and $t_{ip}$ for these four matrices yields Figure~\ref{fig:samples}. We see that the optimal computation time ($t_c = t_l + t_{mv} + t_{ip} \approx t_{mv} + t_{ip}$ as $t_l \approx 0$ by Figure~\ref{fig:nonsig}) seems to lie somewhere between $p=8$ and $p=32$. This result can be explained by the following fact: the systems we used are still too small to scale to a non-shared-memory system. As the Cartesius hardware uses nodes with 12 cores and 2 threads per core, for $p \leq 24$, everything is shared-memory. The solution for this would be to create larger systems. This was however not really possible by the limitations of Mondriaan.
